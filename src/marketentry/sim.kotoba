...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (GRC, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up eu-establishment-missing) ==")
    ...
    (println "== filing/submit eng-4 (eu-establishment-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up afm-unverified) ==")
    ...
    (println "== filing/submit eng-5 (afm-unverified -> HARD hold) ==")
