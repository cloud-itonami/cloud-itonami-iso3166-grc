(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Greece-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Greece-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Greece
                                       public tenders per this
                                       blueprint's own text),
                                       INDEPENDENTLY verify
                                       `:has-eu-establishment?` is
                                       ...
                                       Grounded in EU establishment or Greek
                                       GEMI entity for many procedures
                                       (contracting authorities / EAADHSY).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(AFM/統一資格/ESIDIS/Promitheus登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- eu-establishment-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-eu-establishment? true`, INDEPENDENTLY verify
  `:has-eu-establishment?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-eu-establishment?` ground truth (most Greece public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
                 (not (true? (:has-eu-establishment? e))))
        [{:rule :eu-establishment-missing
          :detail (str subject " はギリシャ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- afm-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-afm? true`, INDEPENDENTLY check
  `:afm-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-afm? e))
                 (not (true? (:afm-verified? e))))
        [{:rule :afm-unverified
          :detail (str subject " はAFM確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (eu-establishment-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (afm-unverified-violations request st)
