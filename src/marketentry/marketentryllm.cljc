(ns marketentry.marketentryllm
  ...
  the GRC public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-eu-establishment?=" (:has-eu-establishment? e)
                        " afm-verified?=" (:afm-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-eu-establishment? e))
                              (:has-eu-establishment? e))
                          (or (not (:requires-afm? e))
                              (:afm-verified? e)))
                   0.9 0.3)}))
